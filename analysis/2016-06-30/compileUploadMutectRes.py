...
#for p in allpats:
allfiles = os.listdir('/mnt/huge/dermalWgs')
snpeffs = [a for a in allfiles if 'snpeff.vcf' in a]
...
#        outfile='patient_%s_tumor_%s_vs_normal_%s.snp'%(p,tu,normind[0])
    ...
    script=base+'2016-06-30/compileUploadMutectRes.py'
    cmdfile=''
   #  vcf = compileVcfs('/mnt/huge/dermalWgs/varDict/'+normsamp+'_'+tumsamp+'.snpeff.vcf',20,cmdfile=cmdfile)
        #vcf = 'test.vcf.gz'
    parts=snpeff.split('_')
    snpeff='/mnt/huge/dermalWgs/'+snpeff
    print parts
    tum=parts[3]
    norm=parts[6].split('.')[0] 
    sv_file = synapseclient.File(snpeff,description='Tumor-normal Chr17 VCF file',parentId='syn6186535')
    sv_act = synapseclient.activity.Activity(name='Mutect and snpeff',
                                                 description = 'Recalculated variants for chrom 17',
                                                used=[tum,norm],executed=script)
    si=syn.store(sv_file,activity=sv_act)
    
    fvcf = applyVcfFilter(snpeff)
        #fvcf = 'test.filtered.vcf.gz'
    sf_file = synapseclient.File(fvcf,description='Tumor-normal Filtered SNPEFF VCF file',parentId='syn6186535')
    sf_act=synapseclient.activity.Activity(name='Filtered vcf from mutect and snpeff',used=[tum,norm,si],executed=script)
    si=syn.store(sf_file,activity=sf_act)        

    maffile = makeMafFromVcf(fvcf,cmdfile=cmdfile)
        #maffile = 'test.maf.gz
    sm_file = synapseclient.File(maffile,description='Tumor-normal filtered MAF file',parentId='syn6186823')
    sm_act=synapseclient.activity.Activity(name='MAF of chr17 from mutect and snpeff',used=[tum,norm,si],executed=script)
