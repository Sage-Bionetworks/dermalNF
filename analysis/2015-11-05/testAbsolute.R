...
run_abs<-function(fname){
    ...
    sname<-paste(annots[match(gsub('X','3086-PBK-',gsub('.seg','_Final.csv',fname)),annots$File),c(2,4)],collapse=' ')
    ...
    RunAbsolute(fname,sample.name=sname,min.ploidy=min.ploidy,max.ploidy=max.ploidy,
                ...
                output.fn.base=sname,primary.disease='PNF',max.as.seg.count=max.as.seg.count,
