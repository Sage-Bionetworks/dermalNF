...
#        outfile='patient_%s_tumor_%s_vs_normal_%s.snp'%(p,tu,normind[0])
	 ...
	 #   continue
        ...
        script=base+'2016-07-27/compileUploadAltResults.py'

        ##first do the varscan files
       	svcf ='/mnt/huge/dermalWgs/varScan/'+tumsamp+'_vs_'+normsamp+'_VarScan_somatic.snp.vcf'
        ivcf = '/mnt/huge/dermalWgs/varScan/'+tumsamp+'_vs_'+normsamp+'_VarScan_somatic.indel.vcf'


        for vcf in [svcf, ivcf]:
            #first do snp file
            sv_file = synapseclient.File(vcf,description='Tumor-normal SNP VCF file',parentId='syn6834340')
            sv_act = synapseclient.activity.Activity(name='Varscan file',
            ...
            #        sf_act=synapseclient.activity.Activity(name='Filtered vcf from varscan and',used=[normind[0],tu,si],executed=script)
        ...
        ##second, do the mutect files
        vcf='patient_%s_tumor_%s_vs_normal_%s.snp_chr17.vcf'%(p,tu,normind[0])
        sv_file = synapseclient.File(vcf,description='Tumor-normal SNP VCF file',parentId='syn6186535')
        sv_act = synapseclient.activity.Activity(name='Mutect file',
                                                 description = 'Recalculated variants from bam files for Chromosome 17',
                                                used=[normind[0],tu],executed=script)
        si=syn.store(sv_file,activity=sv_act)
            #  fvcf = applyVcfFilter(vcf)
