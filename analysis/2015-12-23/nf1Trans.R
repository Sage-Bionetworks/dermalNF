...
p<-ggplot(df)+geom_boxplot(aes(Patient,Counts,border=Patient))+geom_jitter(aes(Patient,Counts,color=Patient))+theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
print(p)
dev.off()