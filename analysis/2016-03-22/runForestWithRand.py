        ...
        for mu in mu_range:
            for beta in beta_range:
                for w in w_range:
                    ...
                    out_label = "%s_w%f_beta%d_D%d_mu%f" %(prefix,w,beta,D,mu)
                    ne_cmd="python %s --prize %s --edge %s --conf conf.txt --noisyEdges 50 --msgpath ~/msgsteiner-1.1/msgsteiner --outpath %s --outlabel %s"%(os.path.join(omics_int_dir,'scripts/forest.py'),prize_file,edge_file,output_path,out_label)
                    print ne_cmd
                    os.system(ne_cmd)
