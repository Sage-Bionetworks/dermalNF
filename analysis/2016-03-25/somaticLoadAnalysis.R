...
dft <- non.silent.tcga%>%group_by(Tumor_Sample_Barcode)%>%summarize(MutatedGenes=n_distinct(Hugo_Symbol))
dft$Disease<-non.silent.tcga$tumor_type[match(dft$Tumor_Sample_Barcode,non.silent.tcga$Tumor_Sample_Barcode)]

dft<-rbind(dft,data.frame(Disease=rep('dermalNF',nrow(som.counts)),Tumor_Sample_Barcode=som.counts$Sample_ID,
