...
mvals<-dft %>% group_by(Disease)%>% summarize(m=mean(MutatedGenes))

dft$Disease<-factor(dft$Disease,levels=mvals$Disease[order(mvals$m)])
...
mvals<-edft %>% group_by(Disease)%>% summarize(m=median(MutatedGenes))

edft$Disease<-factor(edft$Disease,levels=mvals$Disease[order(mvals$m)])
