...
def runMutect(normfile,tumfile,out_prefix):
    ...
    cmd='java -jar ~/muTect/muTect-1.1.4.jar --analysis_type MuTect --reference_sequence %s --cosmic %s \
         --dbsnp %s --intervals %s --input_file:normal %s --input_file:tumor %s --out %s -cov %s'%(reference,cosmic,dbsnp,intervals,normfile,tumfile,out_prefix+'.out',out_prefix+'_coverage.wig.txt')
    print cmd
