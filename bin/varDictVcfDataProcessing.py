...
for outvcf in os.path.listfiles('/mnt/huge/dermalWgs/varDict'):
        if 'vcf' not in outvcf:
                continue
...
#        if(bloodfile==''):
 #           outvcf="patient%s_tumor_%s_only.vcf"%(pat,t)
        outmaf=re.sub('.vcf','.mar',os.path.basename(outvcf))
 ...
 #       annotationstr=annotationstr+",\"tissueID\":\""+tumfile_annotations['tissueID'][0]+"\"}'"
       ...
       # synapse_upload_vcf="synapse store "+outvcf+".gz --parentId=syn5522791 --annotations "+annotationstr#+' --used '+usedstr
        ...
        fdeets=gsub(".vcf","",os.path.basename(outvcf)).split('_')
        pat='patient_'+fdeets[1]
        normid=pat+'_normal_'+fdeets[3]
        tumid=pat+'_tumor_'+fdeets[5]
      #  patsh.write('bgzip -d '+outvcf+'.gz\n\n')
        vcf2maf_cmd=vcf2maf+" --input-vcf %s --vcf-tumor-id %s"%(outvcf,tumid)
        vcf2maf_cmd+=' --vcf-normal-id '+normid
